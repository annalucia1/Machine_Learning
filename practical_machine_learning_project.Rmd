...
```{r}
nav <- sapply(colnames(myTraining), function(x) if(sum(is.na(myTraining[, x])) > 0.8*nrow(myTraining)){return(T)}else{return(F)})
...
From the prediction we can see that the out-of-sample error is ? for decison tree and ? for random forests. It shows that random forests is more accurate. 
